...
describe("POST /api/vehicles", () => {
  ...
  it("should return 401 Unauthorized if Authorization header is missing", async () => {
    const response = await request(app).post("/api/vehicles").send(validPayload);
  ...
  // src/tests/vehicle/vehicle.route.test.ts
  ...
  it("should return 400 Bad Request if invalid category is passed", async () => {
    ...
    expect(response.body.success).toBe(false); // Change status: 'fail' -> success: false
